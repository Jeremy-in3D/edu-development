...
export async function login(
  req: Request<{}, {}, { email?: string; password?: string; orgCode?: string }>,
  res: Response,
) {
  ...
  if (email && password) {
    try {
      const user: any = await mongo.login(email, "users");
      ...
      if (user) {
        ...
        const orgData = await mongo.find(
        ...
        if (email) {
          const insertedTokenToUser = await mongo.updateCollection(
            { email },
            { $set: { token } },
            "users",
          );
