...
import { loginValidateMiddleware, userSchema, } from "./routes/auth/validation/loginValidation.js";
import { signupValidateMiddleware, userSchemaSignup, } from "./routes/auth/validation/signupValidation.js";
...
const clearUploadsMiddleware = (req, res, next) => {
    clearUploadsFolder();
    next();
};
...
async function pollAndStore(modelId) {
    ...
    try {
        ...
        for (let attempts = 0; attempts < 80; attempts++) {
            const statusRes = await fetch(`https://api.tripo3d.ai/v2/openapi/task/${record.tripoTaskId}`, { headers: { Authorization: `Bearer ${tripoKey}` } });
...
app.post("/account", verifyToken, async (req, res) => handleAccountDetails(req, res));
app.post("/active-lessons", verifyToken, async (req, res) => getActiveLessons(req, res));
app.post("/create-lesson", verifyToken, clearUploadsMiddleware, upload.array("files", 10), async (req, res) => createLesson(req, res));
app.post("/join-lesson", verifyToken, async (req, res) => joinLesson(req, res));
app.post("/login", authLimiter, loginValidateMiddleware(userSchema), async (req, res) => login(req, res));
app.post("/signup", authLimiter, signupValidateMiddleware(userSchemaSignup), async (req, res) => signup(req, res));
app.post("/org-members", verifyToken, async (req, res) => getOrgMembers(req, res));
app.post("/upload", verifyToken, upload.single("file"), async (req, res, next) => {
    try {
        ...
        if (result) {
            await mongo.updateCollection({ userId: req.body.userId }, { $set: { profilePic: result.url } }, "users");
        }
...
app.post("/generate-model", async (req, res) => {
    try {
        ...
        let model_url;
        for (let attempts = 0; attempts < 80; attempts++) {
            const statusRes = await fetch(`https://api.tripo3d.ai/v2/openapi/task/${task_id}`, { headers: { Authorization: `Bearer ${tripoKey}` } });
            const statusData = await statusRes.json();
            const taskStatus = statusData.data.status;
